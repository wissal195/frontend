...
export class BlogHelperService implements AfterContentInit, OnInit {
  ...
  public getPostNavigation(items: string | any[], index: number) {
    ...
    if (items[index - 1] !== undefined && index - 1 !== -1) {
      ...
      output.push("<li class='page-item'><a class='page-link' href='/blog-details/" + item.id + "'>Previous</a></li>");
    }
    if (items[index + 1] !== undefined && index <= items.length - 1) {
      // Show next button 
      item = items[index + 1];
      id = item.id;
      output.push("<li class='page-item'><a class='page-link' href='/blog-details/" + item.id + "'>Next</a></li>");
    }
    return output;
  }
  // Social Share
